 ...
 *     <meta name="parachute-mount" content="/surface/<name>">
 ...
 *     nothing else. It does not import from `@openparachute/surface` or
 ...
 * parachute-surface's host. Returns the mount path WITHOUT a trailing slash
...
export function getMountBase(opts?: { doc?: Document | null }): string | null {
  ...
  // Reject bare "/" — the contract requires `/surface/<slug>` or a legacy
  ...
  // `/surface/notes/` and we shouldn't make consumers handle both shapes.
 ...
 *
 * Returns the last segment of `/surface/<name>` (e.g. `"notes"` for
 * `/surface/notes`, `"my-notes"` for `/surface/my-notes`). Returns `null` if
 * the mount doesn't match the `/surface/<slug>` pattern — legacy
...
export function getTenantId(opts?: { doc?: Document | null }): string | null {
  ...
  // Match `/surface/<slug>` exactly — single segment after `/surface/`. Slug
  // grammar mirrors parachute-surface's meta-schema PATH_PATTERN
  // (`[a-z0-9][a-z0-9_-]*`).
  const match = /^\/surface\/([a-z0-9][a-z0-9_-]*)$/.exec(mount);
  return match?.[1] ?? null;
}
