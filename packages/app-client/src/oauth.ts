 ...
 * parachute-app gets its own OAuth client_id (per design doc section 6 —
 * "Per-UI client_id"). The UI's JS reads its `client_id` at boot from
 * `/app/<name>/oauth-client`, then drives the OAuth dance through this
 * class.
 *
 * Lifecycle the class assumes:
 *
 *   1. `getClientId(appName)` — fetches `/app/<name>/oauth-client`,
...
export type ParachuteOAuthOpts = {
   ...
   * and the `<name>` segment in `/app/<name>/oauth-client`. Becomes the
 ...
 * `/app/<name>/oauth-client` endpoint.
 */
...
export class ParachuteOAuth {
  ...
  /**
   * Fetch `/app/<name>/oauth-client`. Cached for the lifetime of this
  ...
  async getClientId(): Promise<OAuthClientInfo> {
    ...
    const endpoint = `${this.hubUrl}/app/${encodeURIComponent(this.appName)}/oauth-client`;
  ...
  private defaultRedirectUri(): string {
    ...
    // top-level path is `/app/<name>/` and the callback lives directly
    // under it — the same shape Notes uses today.
    const origin = typeof window !== "undefined" ? window.location.origin : "";
    return `${origin}/app/${encodeURIComponent(this.appName)}/oauth/callback`;
  }
