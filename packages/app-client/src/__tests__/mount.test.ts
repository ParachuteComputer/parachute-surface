 ...
 *   - getTenantId   → /app/<slug> shapes (single + multi-char-slug) +
...
describe("getMountBase", () => {
  test("reads <meta name=parachute-mount> content", () => {
    ...
    expect(getMountBase({ doc })).toBe("/app/notes");
  });
  ...
  test("preserves multi-char slug names", () => {
    const doc = makeDoc({ "parachute-mount": "/app/my-notes" });
    expect(getMountBase({ doc })).toBe("/app/my-notes");
  });
});

describe("getTenantId", () => {
  test("/app/notes → 'notes'", () => {
    const doc = makeDoc({ "parachute-mount": "/app/notes" });
    expect(getTenantId({ doc })).toBe("notes");
  });

  test("/app/my-notes → 'my-notes' (hyphens allowed)", () => {
    const doc = makeDoc({ "parachute-mount": "/app/my-notes" });
    expect(getTenantId({ doc })).toBe("my-notes");
  });

  test("/app/notes_v2 → 'notes_v2' (underscores allowed)", () => {
    const doc = makeDoc({ "parachute-mount": "/app/notes_v2" });
  ...
  test("/app/foo/bar → null (multi-segment, not the tenant pattern)", () => {
    const doc = makeDoc({ "parachute-mount": "/app/foo/bar" });
    expect(getTenantId({ doc })).toBeNull();
  });

  test("/app → null (missing tenant segment)", () => {
    const doc = makeDoc({ "parachute-mount": "/app" });
    expect(getTenantId({ doc })).toBeNull();
  });
  ...
  test("/app/Notes (uppercase) → null (slug grammar is lowercase)", () => {
    // Matches parachute-app's PATH_PATTERN: `[a-z0-9][a-z0-9_-]*`.
