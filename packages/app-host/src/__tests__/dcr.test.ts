...
describe("registerOauthClient", () => {
  test("sends RFC 7591-shaped body with no auth", async () => {
      ...
      return Promise.resolve(
        new Response(
          JSON.stringify({
            ...
            redirect_uris: ["http://hub/app/test/", "http://hub/app/test/oauth-callback"],
    ...
    const res = await registerOauthClient({
      hubUrl: "http://hub/",
      clientName: "Test UI",
      redirectUris: ["http://hub/app/test/", "http://hub/app/test/oauth-callback"],
  ...
  test("sends Authorization when operatorToken present", async () => {
    ...
    await registerOauthClient({
      hubUrl: "http://hub",
      clientName: "Test",
      redirectUris: ["http://hub/app/test/"],
      scopes: ["vault:*:read"],
