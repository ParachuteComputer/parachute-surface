 ...
 * an optional dev-mode flag the operator toggles via `parachute-surface dev
 ...
 *      an EventSource against `/surface/<name>/_dev/reload`. Operator-triggered
 *      reload events broadcast on the stream cause the tab to reload.
 *   3. The operator-flow trigger is manual at MVP — `parachute-surface dev
 ...
 * `/surface/<name>/_dev/reload`. We keep both the controller (for `enqueue`)
