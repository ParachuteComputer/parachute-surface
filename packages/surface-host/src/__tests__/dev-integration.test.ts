 ...
 *
 * Coverage:
 *   - With dev mode ON, GET /surface/<name>/ injects the reload script
 *   - With dev mode OFF, GET /surface/<name>/ does NOT inject
...
beforeEach(() => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "parachute-surface", version: "0.1.0-rc.4" }),
  );
  fs.writeFileSync(
...
describe("dev mode — end-to-end through HTTP", () => {
  test("dev mode injects the reload script into index.html", async () => {
    ...
    try {
      ...
      const before = await fetch(`${srv.url}/surface/notes/`);
      const beforeText = await before.text();
      expect(beforeText).not.toContain("parachute-app-dev-reload");

      // Enable
      const enable = await fetch(`${srv.url}/surface/notes/dev/enable`, { method: "POST" });
      expect(enable.status).toBe(200);

      // Post-dev: injected.
      const after = await fetch(`${srv.url}/surface/notes/`);
      const afterText = await after.text();
      expect(afterText).toContain(`id="parachute-app-dev-reload"`);
      expect(afterText).toContain(`new EventSource("/surface/notes/_dev/reload")`);
  ...
  test("dev mode overrides hashed-asset immutable header with no-store", async () => {
    ...
    try {
      ...
      const asset = await fetch(`${srv.url}/surface/notes/app.a3b9f2c1.js`);
  ...
  test("dev mode overrides PWA SW cache header as well", async () => {
    ...
    const srv = startServer(makeState([ui]));
    try {
      await fetch(`${srv.url}/surface/notes/dev/enable`, { method: "POST" });
      const sw = await fetch(`${srv.url}/surface/notes/sw.js`);
  ...
  test("HEAD on index.html in dev mode reports injected byte length", async () => {
    ...
    const ui = makeUi("notes", "/surface/notes", { "index.html": html });
    const srv = startServer(makeState([ui]));
    try {
      await fetch(`${srv.url}/surface/notes/dev/enable`, { method: "POST" });
      const head = await fetch(`${srv.url}/surface/notes/`, { method: "HEAD" });
  ...
  test("SSE stream receives a broadcast when trigger fires", async () => {
    ...
    try {
      ...
      const sseRes = await fetch(`${srv.url}/surface/notes/_dev/reload`);
      ...
      const trig = await fetch(`${srv.url}/surface/notes/dev/trigger`, { method: "POST" });
  ...
  test("disable closes active SSE streams", async () => {
    ...
    try {
      ...
      await fetch(`${srv.url}/surface/notes/dev/disable`, { method: "POST" });
  ...
  test("dev list endpoint reflects state", async () => {
    const ui1 = makeUi("foo", "/surface/foo", { "index.html": "<html/>" });
    const ui2 = makeUi("bar", "/surface/bar", { "index.html": "<html/>" });
    const srv = startServer(makeState([ui1, ui2]));
    try {
      // Initially empty
      let listRes = await fetch(`${srv.url}/surface/dev/list`);
      let body = (await listRes.json()) as { uis: Array<{ name: string }> };
      expect(body.uis).toEqual([]);

      // Enable foo
      await fetch(`${srv.url}/surface/foo/dev/enable`, { method: "POST" });

      listRes = await fetch(`${srv.url}/surface/dev/list`);
  ...
  test("re-fetching index in dev mode doesn't duplicate the script", async () => {
    ...
    try {
      await fetch(`${srv.url}/surface/notes/dev/enable`, { method: "POST" });
      const first = await (await fetch(`${srv.url}/surface/notes/`)).text();
      const second = await (await fetch(`${srv.url}/surface/notes/`)).text();
  ...
  test("disabling dev mode restores production headers + no injection", async () => {
    ...
    try {
      await fetch(`${srv.url}/surface/notes/dev/enable`, { method: "POST" });
      const onText = await (await fetch(`${srv.url}/surface/notes/`)).text();
      expect(onText).toContain("parachute-app-dev-reload");

      await fetch(`${srv.url}/surface/notes/dev/disable`, { method: "POST" });
      const off = await fetch(`${srv.url}/surface/notes/`);
