 ...
 *
 * Coverage:
 *   - 401 from /surface/list without bearer (real auth path)
 *   - 200 from /surface/list with auth bypassed
 ...
 *   - /surface/admin/ serves placeholder when no admin bundle is built
 */
...
describe("HTTP-level admin auth", () => {
  test("401 without bearer", async () => {
    ...
    try {
      const r = await fetch(`${srv.url}/surface/list`);
...
describe("end-to-end add / delete / reload", () => {
  test("add a local UI + verify mount serves index.html", async () => {
    ...
    const srv = startServer(state, allowAdmin);
    try {
      const addRes = await fetch(`${srv.url}/surface/add`, {
        method: "POST",
      ...
      const fetched = await fetch(`${srv.url}/surface/e2eui/`);
      expect(fetched.status).toBe(200);
      expect(await fetched.text()).toContain("e2e");

      // Listing surfaces it.
      const listRes = await fetch(`${srv.url}/surface/list`);
  ...
  test("delete a UI + verify mount returns 404", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "doomed", displayName: "Doomed", path: "/surface/doomed" }),
    );
    ...
    try {
      ...
      const before = await fetch(`${srv.url}/surface/doomed/`);
      expect(before.status).toBe(200);

      const delRes = await fetch(`${srv.url}/surface/doomed`, { method: "DELETE" });
      expect(delRes.status).toBe(200);

      // After delete, the mount returns 404 (no matching UI).
      const after = await fetch(`${srv.url}/surface/doomed/`);
  ...
  test("reload picks up meta.json changes", async () => {
    ...
    fs.writeFileSync(
      path.join(dir, "meta.json"),
      JSON.stringify({ name: "rel", displayName: "v1", path: "/surface/rel" }),
    );
    ...
    try {
      ...
      fs.writeFileSync(
        path.join(dir, "meta.json"),
        JSON.stringify({ name: "rel", displayName: "v2", path: "/surface/rel" }),
      );
      const r = await fetch(`${srv.url}/surface/rel/reload`, { method: "POST" });
      expect(r.status).toBe(200);
  ...
  test("/surface/admin/ serves placeholder when bundle absent", async () => {
    const state = makeState();
    const srv = startServer(state, allowAdmin);
    try {
      const r = await fetch(`${srv.url}/surface/admin/`);
      expect(r.status).toBe(200);
      const text = await r.text();
      expect(text).toContain("parachute-surface admin");
    } finally {
      srv.stop();
    }
  });

  test("/surface/admin redirect-equivalent (no trailing slash) also serves the bundle", async () => {
    const state = makeState();
    const srv = startServer(state, allowAdmin);
    try {
      const r = await fetch(`${srv.url}/surface/admin`);
...
describe("/surface/<name>/oauth-client (unauthenticated through HTTP)", () => {
  test("UI without oauth client → 404", async () => {
    const dir = path.join(uisDir, "noauth");
    fs.mkdirSync(path.join(dir, "dist"), { recursive: true });
    fs.writeFileSync(
      path.join(dir, "meta.json"),
      JSON.stringify({ name: "noauth", displayName: "noauth", path: "/surface/noauth" }),
    );
    fs.writeFileSync(path.join(dir, "dist", "index.html"), "x");
    const state = makeState();
    const srv = startServer(state, allowAdmin);
    try {
      const r = await fetch(`${srv.url}/surface/noauth/oauth-client`);
