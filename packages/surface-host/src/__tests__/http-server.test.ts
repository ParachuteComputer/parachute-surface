 ...
 *
 * Coverage:
 *   - /healthz + /surface/healthz both 200 with UI counts
...
beforeEach(() => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "parachute-surface", version: "0.1.0-rc.2" }),
  );
  fs.writeFileSync(
...
describe("HTTP — healthz + .parachute/*", () => {
  test("/healthz returns 200 with UI count", async () => {
    const ui = makeUi("notes", "/surface/notes", { "index.html": "<!doctype html>" });
  ...
  test("/surface/healthz mirrors /healthz", async () => {
    const state = makeState([]);
    const srv = startServer(state);
    try {
      const r = await fetch(`${srv.url}/surface/healthz`);
      expect(r.status).toBe(200);
      const body = (await r.json()) as { status: string; uis: number };
      expect(body.status).toBe("ok");
  ...
  test("/.parachute/info served from parachuteDir", async () => {
    ...
    try {
      ...
      expect(body.name).toBe("parachute-surface");
    } finally {
      srv.stop();
    }
  });
...
describe("HTTP — UI mount paths", () => {
  test("GET /<path>/ serves index.html with no-cache", async () => {
    const ui = makeUi("notes", "/surface/notes", { "index.html": "Hello Notes" });
    const srv = startServer(makeState([ui]));
    try {
      const r = await fetch(`${srv.url}/surface/notes/`);
      expect(r.status).toBe(200);
  ...
  test("GET /<path>/<hashed-asset> serves with immutable header", async () => {
    const ui = makeUi("notes", "/surface/notes", {
      "index.html": "i",
      "app.a3b9f2c1.js": "console.log('hi')",
    });
    const srv = startServer(makeState([ui]));
    try {
      const r = await fetch(`${srv.url}/surface/notes/app.a3b9f2c1.js`);
      expect(r.status).toBe(200);
  ...
  test("GET /<path>/<non-hashed-asset> serves with 1-hour cache", async () => {
    ...
    const srv = startServer(makeState([ui]));
    try {
      const r = await fetch(`${srv.url}/surface/notes/icon.svg`);
      expect(r.status).toBe(200);
  ...
  test("SPA fallback: unknown subpath under mount serves index.html", async () => {
    const ui = makeUi("notes", "/surface/notes", { "index.html": "Hello" });
    const srv = startServer(makeState([ui]));
    try {
      const r = await fetch(`${srv.url}/surface/notes/some/spa/route`);
      expect(r.status).toBe(200);
      expect(r.headers.get("content-type")).toContain("text/html");
      expect(r.headers.get("cache-control")).toBe("no-cache, no-store, must-revalidate");
  ...
  test("PWA opt-in: SW served with no-cache", async () => {
    ...
    const srv = startServer(makeState([ui]));
    try {
      const r = await fetch(`${srv.url}/surface/notes/sw.js`);
      expect(r.status).toBe(200);
  ...
  test("HEAD returns headers + no body", async () => {
    ...
    const srv = startServer(makeState([ui]));
    try {
      const r = await fetch(`${srv.url}/surface/notes/app.a3b9f2c1.js`, { method: "HEAD" });
      expect(r.status).toBe(200);
      expect(r.headers.get("cache-control")).toBe("public, max-age=31536000, immutable");
  ...
  test("path traversal attempt with asset extension → 404 (defense in depth)", async () => {
    ...
    const ui = makeUi("notes", "/surface/notes", { "index.html": "safe" });
    // Write a sensitive file in the parent
    fs.writeFileSync(path.join(tmpDir, "secret.txt"), "very secret");
    const srv = startServer(makeState([ui]));
    try {
      const r = await fetch(`${srv.url}/surface/notes/..%2Fsecret.txt`);
  ...
  test("path traversal attempt with no extension → SPA fallback (still defended)", async () => {
    ...
    const srv = startServer(makeState([ui]));
    try {
      const r = await fetch(`${srv.url}/surface/notes/..%2Fsecret`);
      expect(r.status).toBe(200);
  ...
  describe("asset-vs-navigation miss policy", () => {
    ...
    test("missing .js asset returns 404 (not SPA shell)", async () => {
      const ui = makeUi("notes", "/surface/notes", { "index.html": "SPA SHELL" });
      const srv = startServer(makeState([ui]));
      try {
        const r = await fetch(`${srv.url}/surface/notes/missing-chunk.js`);
        expect(r.status).toBe(404);
        const body = await r.text();
        expect(body).toBe("Not Found");
    ...
    test("missing .webmanifest returns 404 (not SPA shell)", async () => {
      ...
      const srv = startServer(makeState([ui]));
      try {
        const r = await fetch(`${srv.url}/surface/notes/manifest.webmanifest`);
        expect(r.status).toBe(404);
    ...
    test("missing .mjs asset returns 404 (not SPA shell)", async () => {
      ...
      const srv = startServer(makeState([ui]));
      try {
        const r = await fetch(`${srv.url}/surface/notes/assets/chunk.mjs`);
        expect(r.status).toBe(404);
        expect(await r.text()).toBe("Not Found");
      } finally {
        srv.stop();
    ...
    test("missing .css asset returns 404 (not SPA shell)", async () => {
      ...
      const srv = startServer(makeState([ui]));
      try {
        const r = await fetch(`${srv.url}/surface/notes/styles/missing.css`);
        expect(r.status).toBe(404);
      } finally {
        srv.stop();
      }
    ...
    test("missing path with no extension serves SPA shell (client-side route)", async () => {
      ...
      const srv = startServer(makeState([ui]));
      try {
        const r = await fetch(`${srv.url}/surface/notes/some/route/without/extension`);
        expect(r.status).toBe(200);
        expect(r.headers.get("content-type")).toContain("text/html");
    ...
    test("missing path with .html extension serves SPA shell", async () => {
      ...
      const srv = startServer(makeState([ui]));
      try {
        const r = await fetch(`${srv.url}/surface/notes/route.html`);
        expect(r.status).toBe(200);
        expect(r.headers.get("content-type")).toContain("text/html");
        expect(await r.text()).toBe("SPA SHELL");
      } finally {
    ...
    test("missing bare segment (e.g. /surface/notes/notes) serves SPA shell", async () => {
      const ui = makeUi("notes", "/surface/notes", { "index.html": "SPA SHELL" });
      const srv = startServer(makeState([ui]));
      try {
        const r = await fetch(`${srv.url}/surface/notes/notes`);
        expect(r.status).toBe(200);
        expect(await r.text()).toBe("SPA SHELL");
      } finally {
        srv.stop();
    ...
    test("present .js asset still serves correctly (regression guard)", async () => {
      ...
      const srv = startServer(makeState([ui]));
      try {
        const r = await fetch(`${srv.url}/surface/notes/real.js`);
        expect(r.status).toBe(200);
        expect(r.headers.get("content-type")).toContain("javascript");
        expect(await r.text()).toBe("console.log('hi')");
      } finally {
  ...
  test("two UIs at different mounts", async () => {
    const ui1 = makeUi("foo", "/surface/foo", { "index.html": "FOO" });
    const ui2 = makeUi("bar", "/surface/bar", { "index.html": "BAR" });
    const srv = startServer(makeState([ui1, ui2]));
    try {
      const r1 = await fetch(`${srv.url}/surface/foo/`);
      expect(await r1.text()).toBe("FOO");
      const r2 = await fetch(`${srv.url}/surface/bar/`);
...
describe("HTTP — runtime tenancy contract injection", () => {
  ...
  test("root document GET /surface/notes/ — tags injected", async () => {
    const ui = makeUi("notes", "/surface/notes", { "index.html": HEAD_HTML });
    const srv = startServer(makeState([ui]));
    try {
      const r = await fetch(`${srv.url}/surface/notes/`);
      expect(r.status).toBe(200);
      const body = await r.text();
      expect(body).toContain('<base href="/surface/notes/">');
      expect(body).toContain('<meta name="parachute-mount" content="/surface/notes">');
  ...
  test("GET /surface/notes (no trailing slash) — tags also injected", async () => {
  ...
  test("custom mount slug — <base href> + parachute-mount reflect it", async () => {
    // Operators can `parachute-surface add @openparachute/notes-ui --name my-notes`
    // to mount a UI at /surface/my-notes. The bundle is identical; only the host's
    // injected metadata reveals the chosen mount.
    const ui = makeUi("my-notes", "/surface/my-notes", { "index.html": HEAD_HTML });
    const srv = startServer(makeState([ui]));
    try {
      const r = await fetch(`${srv.url}/surface/my-notes/`);
      expect(r.status).toBe(200);
      const body = await r.text();
      expect(body).toContain('<base href="/surface/my-notes/">');
      expect(body).toContain('<meta name="parachute-mount" content="/surface/my-notes">');
    } finally {
      srv.stop();
    }
  });

  test("SPA-fallback path /surface/notes/some/route — also injects", async () => {
  ...
  test("idempotent — bundle that already declares parachute-mount is served unchanged", async () => {
    ...
    const ui = makeUi("notes", "/surface/notes", { "index.html": pre });
    const srv = startServer(makeState([ui]));
    try {
      const r = await fetch(`${srv.url}/surface/notes/`);
      expect(r.status).toBe(200);
  ...
  test("malformed (no <head>) index.html — served unmodified", async () => {
    ...
    const ui = makeUi("notes", "/surface/notes", { "index.html": malformed });
    const srv = startServer(makeState([ui]));
    try {
      const r = await fetch(`${srv.url}/surface/notes/`);
      expect(r.status).toBe(200);
  ...
  test("non-index assets are NOT touched (regression guard)", async () => {
    ...
    const srv = startServer(makeState([ui]));
    try {
      const r = await fetch(`${srv.url}/surface/notes/app.js`);
      expect(r.status).toBe(200);
...
describe("HTTP — 404 + 405", () => {
  ...
  test("404 under /surface but no matching UI", async () => {
    const srv = startServer(makeState([]));
    try {
      const r = await fetch(`${srv.url}/surface/notes/`);
      expect(r.status).toBe(404);
  ...
  test("DELETE to a non-admin route returns 404", async () => {
    // /not-an-app/x doesn't match the `/surface/<name>` admin DELETE regex.
    const srv = startServer(makeState([]));
    try {
