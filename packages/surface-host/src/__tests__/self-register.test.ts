 ...
 * `parachute-surface serve` boot. Mirrors runner's coverage shape.
 *
 * Coverage:
...
describe("selfRegister — first boot", () => {
  test("writes a fresh services.json with our entry", () => {
    const result = selfRegister({
      boundPort: 1946,
      installDir: "/Users/x/parachute-surface",
      manifestPath,
      logger,
    });
    ...
    // here would race the hub-installed `parachute-surface` row and trip the
    // duplicate-port detector.
    expect(entry.name).toBe("parachute-surface");
    expect(entry.port).toBe(1946);
    expect(entry.paths).toEqual(["/surface", "/.parachute"]);
    expect(entry.health).toBe("/surface/healthz");
    expect(entry.installDir).toBe("/Users/x/parachute-surface");
  ...
  test("regression: row key matches the manifestName hub installs under (no duplicate `app` row)", () => {
    ...
    // manifest.manifestName ("parachute-surface"). If self-register writes
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        services: [
          {
            name: "parachute-surface", // hub-installed row
            port: 1946,
            paths: ["/surface"],
    ...
    expect(raw.services[0]?.name).toBe("parachute-surface");
...
describe("selfRegister — subsequent boot (existing entry)", () => {
  ...
  test("hub-stamped fields survive the merge", () => {
        ...
        services: [
          {
            ...
            health: "/surface/healthz",
            version: "old",
            uiUrl: "/surface/admin/", // hub-stamped
    ...
    const entry = raw.services[0]!;
    expect(entry.uiUrl).toBe("/surface/admin/");
    expect(entry.managementUrl).toBe("/surface/admin/");
  });
});
...
describe("selfRegister — extraFields (Phase 1.2 hook)", () => {
  test("uis map is stamped onto the row", () => {
    selfRegister({
      ...
      extraFields: {
        uis: {
          notes: { displayName: "Notes", path: "/surface/notes" },
        },
      },
    });
    const raw = JSON.parse(fs.readFileSync(manifestPath, "utf8")) as {
...
describe("selfRegister — invalid-port guard (parachute-app#33 regression)", () => {
  ...
  test("self-heal: existing row with port=0 is replaced by boundPort on restart", () => {
    ...
    const m = JSON.parse(fs.readFileSync(manifestPath, "utf8"));
    const row = m.services.find((s: { name: string }) => s.name === "parachute-surface");
