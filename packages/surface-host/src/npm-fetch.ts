/**
 * npm-fetch shorthand for `POST /surface/add`.
 *
 ...
 * under `~/.parachute/surface/uis/<name>/dist/`, and copies `meta.json` if the
 * package ships one.
 *
 ...
 *
 * The caller (admin-routes `/surface/add`) cleans up the staging dir in a
...
export async function fetchNpmPackage(opts: NpmFetchOpts): Promise<NpmFetchResult> {
  ...
  if (!existsSync(distPath)) {
    ...
    throw new NpmFetchError(
      ...
      {
        retryHint:
          "the package should publish a `dist/` directory containing `index.html`; ask the maintainer or build locally + `parachute-surface add ./path/to/dist`",
      },
    );
  }
 ...
 * Mirror of `cp -r src/. dest/` semantics. Used by `POST /surface/add` to copy
