/**
 * Operator bearer sourcing for outbound calls from parachute-surface to hub.
 *
 ...
 *
 * Apps wants approved (so a `parachute-surface add` is one command, not "add
 ...
 * fatal (DCR auto-register on `parachute-surface add` with `auto_register_oauth_clients=true`)
 * or fine (the `parachute-surface list` path needs no outbound calls).
 */
