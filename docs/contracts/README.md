...
Note: the docs are verbatim copies (plus a provenance header), so relative links they carry were written for the patterns repo's layout and are **dead in this repo** — they do not redirect anywhere. For links to patterns that did **not** move (e.g. `./module-surfaces.md`, `./surface-bundle-shape.md`, `./tag-scoped-tokens.md`), find the file in the read-only archive: <https://github.com/ParachuteComputer/parachute-patterns/tree/main/patterns>. For hub-enforced targets (e.g. `./hub-module-boundary.md`, `./module-protocol.md`), the living copy is in [`parachute-hub/docs/contracts/`](https://github.com/ParachuteComputer/parachute-hub/tree/main/docs/contracts). (Kept verbatim on purpose: editorial fixes to a moved contract are their own PRs, not part of the move.)
