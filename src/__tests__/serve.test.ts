...
describe("serve — live integration", () => {
  test("mounts a UI and serves index.html", async () => {
    seedUi("test-ui", "/app/test-ui", { "index.html": "Hello from test UI" });
    const h = serve({
      port: 0,
      configPath,
      uisDir,
    ...
    try {
      ...
      const r2 = await fetch(`${url}/app/test-ui/app.deadbeef.js`);
      expect(r2.status).toBe(200);
      expect(r2.headers.get("cache-control")).toBe("public, max-age=31536000, immutable");

      // SPA fallback
      const r3 = await fetch(`${url}/app/test-ui/some/route`);
  ...
  test("self-registers into services.json", async () => {
    seedUi("test-ui", "/app/test-ui", { "index.html": "x" });
    const h = serve({
      port: 0,
      configPath,
      uisDir,
...
describe("runOnce", () => {
  test("returns scan result without binding a port", () => {
    seedUi("a", "/app/a", { "index.html": "a" });
    seedUi("b", "/app/b", { "index.html": "b" });
