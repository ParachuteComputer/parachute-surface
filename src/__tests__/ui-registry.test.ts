 ...
 *   - Reserved path /app/admin → skipped (reserved-path)
...
describe("scanUis — discovery", () => {
  ...
  test("one valid UI", () => {
    seedUi("notes", { name: "notes", displayName: "Notes", path: "/app/notes" });
    const result = scanUis({ uisDir, logger: silentLogger });
...
describe("scanUis — skip-and-warn", () => {
  ...
  test("missing dist/index.html", () => {
    seedUi(
      "notes",
      { name: "notes", displayName: "Notes", path: "/app/notes" },
      {
        skipDist: true,
      },
    );
  ...
  test("schema-invalid meta.json", () => {
    seedUi("notes", { name: "Notes", displayName: "X", path: "/app/x" }); // uppercase name
    const result = scanUis({ uisDir, logger: silentLogger });
    expect(result.registered).toHaveLength(0);
    expect(result.skipped).toHaveLength(1);
    expect(result.skipped[0]!.status).toBe("invalid-meta");
  ...
  test("reserved path /app/admin", () => {
    seedUi("admin", { name: "admin", displayName: "Admin", path: "/app/admin" });
    const result = scanUis({ uisDir, logger: silentLogger });
    expect(result.registered).toHaveLength(0);
    expect(result.skipped).toHaveLength(1);
...
describe("scanUis — collision resolution", () => {
  test("alphabetical-by-name wins; loser demoted to collision", () => {
    seedUi("brain-zebra", { name: "brain-zebra", displayName: "Zebra", path: "/app/brain" });
    seedUi("brain-alpha", { name: "brain-alpha", displayName: "Alpha", path: "/app/brain" });
    const result = scanUis({ uisDir, logger: silentLogger });
    expect(result.registered).toHaveLength(1);
  ...
  test("three-way collision: alphabetical-by-name wins", () => {
    seedUi("c", { name: "c", displayName: "C", path: "/app/x" });
    seedUi("a", { name: "a", displayName: "A", path: "/app/x" });
    seedUi("b", { name: "b", displayName: "B", path: "/app/x" });
    const result = scanUis({ uisDir, logger: silentLogger });
    expect(result.registered).toHaveLength(1);
  ...
  test("no collision: different paths", () => {
    seedUi("foo", { name: "foo", displayName: "Foo", path: "/app/foo" });
    seedUi("bar", { name: "bar", displayName: "Bar", path: "/app/bar" });
    const result = scanUis({ uisDir, logger: silentLogger });
...
describe("scanUis — ordering", () => {
  test("registered list is sorted by mount path", () => {
    seedUi("zeta", { name: "zeta", displayName: "Z", path: "/app/zeta" });
    seedUi("alpha", { name: "alpha", displayName: "A", path: "/app/alpha" });
    seedUi("middle", { name: "middle", displayName: "M", path: "/app/middle" });
    const result = scanUis({ uisDir, logger: silentLogger });
